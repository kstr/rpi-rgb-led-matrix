...
public class RGBLedFont : IDisposable
{
    ...
    internal int DrawTextToCanvas(RGBLedCanvas canvas, int x, int y, Color color, string text, int spacing = 0, bool vertical = false)
    {
        ...
        Bindings.SetPixelCallback callback = (px, py, r, g, b) =>
        {
            canvas.SetPixel(px, py, new Color(r, g, b));
        };

        return Bindings.draw_text_callback(_font, x, y, color.R, color.G, color.B, text, spacing, callback, canvas.Width, canvas.Height);
    }
